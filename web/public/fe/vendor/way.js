(function(root, factory) {
  ...
  WAY.prototype.setValue = function(data, options, element) {
    ...
    var defaultSetter = function(a) {
      if (options.html) {
        w.dom(element).html(a);
      }else if (options.mustache){
      	var ele=w.dom(element);
      	var html = ele.html();
      	html = Mustache.render(html,a);
      	var wrapper = document.createElement('div');
      	w.dom(wrapper).html(html);
      	var parent = ele.get(0).parentNode;
      	parent.insertBefore(wrapper, ele.get(0));      	
      }else {
        w.dom(element).text(a);
      }
    };
  ...
  WAY.prototype.bindEvents = function(page) {
  	var self=this;
  	var selector = '[' + tagPrefix + '-click]';
    ...
    for (var i in elements) {
    	...
    	$(element).off("click");
    	if (page[options.click]!=null){
    	element.onclick=page[options.click];
    	}else{
    		error.push(options.click+":function(e){var self=this;alert(JSON.stringify(e));},");
    	}
    }
  ...
  WAY.prototype.registerTransforms = function(page) {
  	var self=this;
  	var selector = '[' + tagPrefix + '-transform]';
  	var elements = w.dom(selector).get();
  	var error=[];
    for (var i in elements) {
    	var element = elements[i];
    	var options=self.dom(element).getOptions();
    	for(var j in options.transform){
    		var t=options.transform[j];
    		if (page[t]!=null){
    			way.registerTransform(t, page[t]);
    		}else{
    			error.push(t+":function(data){},");
    		}
    	}
    }
  ...
  WAY.prototype.setPage = function(page) {
  	var self=this;
		self.set("data", page.data);
  	self.bindEvents(page);
  	self.registerTransforms(page);
  }
    ...
    for (var i in elements) {
      ...
      if (old){
        ...
        w.dom(element).replaceWith(wrapper);
        	...
        	w.dom(element).attr(tagPrefix + '-repeat-wrapper', self._repeatsCount);
          w.dom(element).attr(tagPrefix + '-scope', options.repeat);
          w.dom(element).removeAttr(tagPrefix + '-repeat');
          w.dom(element).html("");
      }
  ...
  WAY.prototype.updateRepeats = function(selector) {
    ...
    repeats.forEach(function(repeat) {
      ...
      var repeatedHtml = w.dom(repeat.element).html();
      for (var key in data) {
        /*
				var item = data[key],
						test = true;
        ...
        w.dom(scope).attr(tagPrefix + '-scope', key);
        w.dom(scope).html(repeatedHtml);
        var html = w.dom(scope).get(0).outerHTML;
        html = html.replace(/\{\{index\}\}/gi, key);
        html = html.replace(/\{\{item\}\}/gi, data[key]);
        html = Mustache.render(html,data[key]);
        items.push(html);
      }
